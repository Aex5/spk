...
export default function Quisioner() {
  const router = useRouter();
  const [questions, setQuestions] = useState([
    { type: 'distance', question: 'Apakah Anda ingin destinasi yang dekat dengan Anda?', value: '' },
    { type: 'rating', question: 'Seberapa penting rating destinasi bagi Anda?', value: '' },
    { type: 'harga_tiket', question: 'Apakah Anda mencari destinasi dengan harga tiket yang murah?', value: '' },
    { type: 'jumlah_pengunjung', question: 'Apakah Anda ingin destinasi dengan jumlah pengunjung yang ramai?', value: '' }
  ]);
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">Destination Recommender</h1>
        ...
        <div className="mt-8">
          ...
          <ul className="space-y-4 mt-4">
            {recommendations.map((destination) => (
              <li key={destination.id} className="border p-4 rounded-md shadow-sm">
                <h3 className="text-lg font-semibold">{destination.destination_name}</h3>
                ...
                <img src={destination.image} alt={destination.destination_name} className="w-full h-auto mt-2" />
              </li>
            ))}
          </ul>
        </div>
